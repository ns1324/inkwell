...
### Design decisions
- **Post.status uses fixed values (DRAFT, PUBLISHED), not free text.** 
Catches typos like "publised" early. Tradeoff: adding ARCHIVED later requires a schema change.
...
Keeps User focused on representing user data; hashing method can change later without touching User.